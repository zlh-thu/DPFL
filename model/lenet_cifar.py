...
class LeNet_Cifar(nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = nn.Conv2d(args.num_channels, 12, kernel_size=5, padding=5 // 2, stride=2)
        self.act1 = act()
        ...
        self.conv1_output_shape = [12, 16, 16]
        self.conv2_output_shape = [12, 8, 8]
        self.conv3_output_shape = [12, 8, 8]
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        self.conv3_output_shape = x.shape
        x = self.act3(x)
    ...
    def reset(self):
        ...
        self.layer_conv3_output = np.zeros(self.conv3_output_shape)
    ...
    def activityNum_forward(self, x):
        ...
        x = self.conv3(x)
        have_sum = np.sum(np.where(x.cpu().detach().numpy() > 0, 1, 0), axis=0)
        self.layer_conv3_output = have_sum + self.layer_conv3_output
        x = self.act3(x)
        ...
        x = self.fc1(x)
        self.layer_fc1_output_lip = np.mean(x.cpu().detach().numpy()) + self.layer_fc1_output_lip
        have_sum = np.sum(np.where(x.cpu().detach().numpy() > 0, 1, 0), axis=0)
    ...
    def get_pogz(self, data_num):
        ...
        self.POGZ_conv3 = np.sum(self.layer_conv3_output) / (data_num * self.layer_conv3_output.size)
        self.POGZ_fc1 = np.sum(self.layer_fc1_output) / (data_num * self.layer_fc1_output.size)

        POGZ_sum = self.POGZ_conv1 + self.POGZ_conv2 + self.POGZ_conv3 + self.POGZ_fc1

        if POGZ_sum != 0:
            self.POGZ_conv1 = self.POGZ_conv1 / POGZ_sum
            self.POGZ_conv2 = self.POGZ_conv2 / POGZ_sum
            self.POGZ_conv3 = self.POGZ_conv3 / POGZ_sum
    ...
    def add_noise(self, POGZ_list, weights, device, sensitivity, epsilon):
        ...
        weights['conv2.weight'] = weights['conv2.weight'] + torch.from_numpy(
            np.random.laplace(loc=0, scale=scale_conv3, size=1)).to(device).float()
